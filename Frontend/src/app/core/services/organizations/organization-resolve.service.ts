...
/** When global mock is on, organizations still use the API if this flag is true. */
export function useOrganizationsFromApi(): boolean {
  const live = (environment as { useLiveOrganizationsApi?: boolean }).useLiveOrganizationsApi === true;
  if (live) return true;
  if ((environment as { persistMarketplaceLeadsToDb?: boolean }).persistMarketplaceLeadsToDb === true) {
