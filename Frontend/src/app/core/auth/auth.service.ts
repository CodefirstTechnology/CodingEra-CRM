...
interface LoginApiResponse {
  access_token?: string;
  token?: string;
  user?: Partial<UserSession> & { Id?: string | number };
...
export class AuthService {
    ...
    return this.http
      ...
      .pipe(
        ...
        switchMap((res) => {
          ...
          const fromLogin =
            pickNumericFromLoginResponse(res) ??
            this.tryNumericUserIdFromJwt(token) ??
            this.tryNumericUserIdFromJwtLoose(token);

          const idAfterList$ = fromLogin
            ? of<string | null>(fromLogin)
            : this.resolveNumericUserIdFromUsersList(base, token, emailResolved);

          return idAfterList$.pipe(
            switchMap((fromUsersList) => {
              const finalNumericId =
                pickNumericDbUserId(fromLogin) ??
                pickNumericDbUserId(fromUsersList) ??
                getEnvQueryUserIdFallback();
              ...
              if (!finalNumericId) {
                this.toast.show(
                  'Signed in, but no numeric user id was found. Return `userId` from login, add a JWT `userId` claim, rely on GET /auth/users to match your email, or set `apiQueryUserIdFallback` in environment.development.ts.',
                  14_000,
                );
                writeLoginLog('login_success', {
                  ...
                  maskedEmail: maskEmail(user.email),
                  detail: 'numeric_user_id_missing',
                });
              ...
              return this.http
                .get<Record<string, unknown>>(
                  `${base}/auth/users/${encodeURIComponent(finalNumericId)}`,
                  {
                ...
                .pipe(
                  ...
                  map((raw) => {
                    ...
                    if (enriched) {
                      const resolved =
                        pickNumericDbUserId(enriched.id) ?? finalNumericId ?? '';
                      this.setSession(token, { ...enriched, id: resolved });
                    }
                    const session = this._user()!;
                    writeLoginLog('login_success', {
                      mode: 'api',
                      userId: session.id,
  ...
  /**
   * When login omits numeric user id, find it from `GET …/auth/users` (email match, or single row).
   */
  private resolveNumericUserIdFromUsersList(
    base: string,
    token: string,
    emailWanted: string,
  ): Observable<string | null> {
    const want = emailWanted.trim().toLowerCase();
    return this.http
      .get<unknown>(`${base}/auth/users`, {
        headers: new HttpHeaders({ Authorization: `Bearer ${token}` }),
      ...
      .pipe(
        timeout(15000),
        map((body) => {
          const rows = this.extractAuthUserRecords(body);
          for (const raw of rows) {
            if (!raw || typeof raw !== 'object') continue;
            const o = raw as Record<string, unknown>;
            const emRaw = o['email'] ?? o['Email'];
            const em = typeof emRaw === 'string' ? emRaw.trim().toLowerCase() : '';
            if (em && em === want) {
              const id = pickNumericDbUserId(o['id'] ?? o['Id'] ?? o['userId'] ?? o['UserId']);
              if (id) return id;
            }
          }
          if (rows.length === 1) {
  ...
  private extractAuthUserRecords(body: unknown): unknown[] {
    if (Array.isArray(body)) return body;
    if (body && typeof body === 'object') {
      const o = body as Record<string, unknown>;
      for (const k of ['data', 'users', 'items', 'results', 'value', 'Values']) {
        const v = o[k];
  ...
  /** Parses JWT payload (no verification) for common `userId` / nameidentifier claims. */
  private tryNumericUserIdFromJwt(token: string): string | null {
    ...
    for (const k of claimKeys) {
      const n = pickNumericDbUserId(payload[k]);
      if (n) return n;
    }
    return pickNumericDbUserId(payload['sub']);
  }

  /**
   * Last resort: any JWT claim whose key suggests user id, or any value that is a short positive int string.
   */
  private tryNumericUserIdFromJwtLoose(token: string): string | null {
    const payload = parseJwtPayload(token);
    if (!payload) return null;
    for (const [k, v] of Object.entries(payload)) {
      if (
        /user\s*id|^userid$/i.test(k) ||
        k.toLowerCase().includes('nameidentifier') ||
        (k.includes('identity') && k.includes('claims'))
      ) {
        const n = pickNumericDbUserId(v);
        if (n) return n;
      }
